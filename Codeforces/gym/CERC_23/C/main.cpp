...
// https://github.com/nealwu/competitive-programming/blob/master/flow/projects_and_tools.cc
...
int main() {
    ...
    vector<int64_t> cake(C);
    for (int i = 0; i < C; ++i) {
        cin >> cake[i];
    }

    vector<int64_t> costIng(G);
    for (int i = 0; i < G; ++i) {
        cin >> costIng[i];
    }

    vector<int64_t> tools(T);
    for (int i = 0; i < T; ++i) {
        cin >> tools[i];
    }

    vector a(C, vector<int64_t>(G));
    for (int i = 0; i < C; ++i) {
        for (int j = 0; j < G; ++j) {
            cin >> a[i][j];
            cake[i] -= a[i][j] * costIng[j];
        }
    }


    projects_and_tools<int64_t> F(cake, tools);
    for (int i = 0; i < C; ++i) {
        int k;
        cin >> k;
        for (int j = 0; j < k; ++j) {
