...
int main() {
    ...
    auto ok = [&](int mask) {
        return __builtin_popcount(mask) % 2 == 0;
    };
    ...
    for (int i = 0; i < n; ++i) {
        ...
        for (int mask = 0; mask < 8; ++mask) {
            for (int len = 0; len < 3; ++len) {
                ...
                if (ok(newMask) || len + 1 <= 2) {
                    ndp[newMask][len + 1] += dp[mask][len];
                } 
            }
